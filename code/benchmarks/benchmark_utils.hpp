...
#define REGISTER_DOUBLE_MT(NAME, DIM)                                 \
    benchmark::RegisterBenchmark(#NAME, (BM_MatMul<double, &(NAME)>)) \
      ->Arg((DIM))                                                    \
      ->UseRealTime()                                                 \
...
#define REGISTER_BF16(NAME, DIM) \
    benchmark::RegisterBenchmark(#NAME, (BM_MatMul<mm::bfloat16_t, &(NAME)>))->Arg((DIM))

#define REGISTER_BF16_MT(NAME, DIM)                                           \
    benchmark::RegisterBenchmark(#NAME, (BM_MatMul<mm::bfloat16_t, &(NAME)>)) \
      ->Arg((DIM))                                                            \
      ->UseRealTime()
